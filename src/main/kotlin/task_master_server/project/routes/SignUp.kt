...
fun Route.signUp(mongoDbService: MongoDbService) {
    post("/signUp") {
        val originalCall = call
        val credentials = originalCall.receive<FirebaseRequestBody>()

        try {
            val firebaseResponse = firebaseSignUpRequest(mongoDbService, credentials)
            if (firebaseResponse.error == null) {
                ...
                mongoDbService.addUser(userDetails)
...
private suspend fun firebaseSignUpRequest(mongoDbService: MongoDbService, credentials: FirebaseRequestBody): FirebaseResponseBody {
    HttpClient(CIO) {
        install(Logging)
        ...
        val response: HttpResponse = it.request("https://identitytoolkit.googleapis.com/v1/accounts:signUp") {
            ...
            parameter("key", "AIzaSyAj3ZRxEEekL2EON6YVb2tXeF3P8BRHV6w")
//            url {
